...
export async function POST(request: NextRequest){
    try {
        ...
        if(!user){
            return NextResponse.json({error: SERVER_MESSAGES.USER_NOT_FOUND}, {status: 400})
        }
        console.log("user exists");
        
        if(!user.isVerified) {
            return NextResponse.json({error: SERVER_MESSAGES.USER_NOT_VERIFIED}, {status: 400})
        }
        
        //check if password is correct
        const validPassword = await bcrypt.compare(password, user.password)
        if(!validPassword){
            return NextResponse.json({error: SERVER_MESSAGES.INVALID_PASSWORD}, {status: 400})
        }
        console.log(user);
